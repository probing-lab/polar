...
class ExactInferenceQuery(Query):
    ...
    def __init__(self, query: str, network: BayesNetwork):
        ...
        if len(parts) != 2:
            raise QueryException(
                "Illegal Format for Exact Inference Query, use VAR**k | VAR = VAL, VAR = VAL, .."
            )
        ...
        if len(target_expr) == 1:
            ...
            raise QueryException(
                f"Invalid query {parts[0].strip()} in Exact Inference Query"
            )

        if self.target_variable not in network.variables:
            raise QueryException(
                f"Unknown variable {self.target_variable} in Exact Inference Query"
            )
        ...
        for evidence_assignment in evidence_list:
            ...
            if evidence_var not in network.variables:
                raise QueryException(
                    f"Unknown variable {evidence_var} in Exact Inference Query"
                )
            if evidence_value not in network.variables[evidence_var].domain:
                raise QueryException(
                    f"Unknown value {evidence_value} in Exact Inference Query"
                )
    ...
    def generate_init_code(self, network, polar_variable_mapping) -> List[str]:
        ...
        self.indicator_name = get_unique_name(
            polar_variable_mapping.values(), "ind_" + target_var_polar_name
        )
        self.inference_name = get_unique_name(
            polar_variable_mapping.values(), "inf_" + target_var_polar_name
        )
    ...
    def generate_loop_code(self, network, polar_variable_mapping) -> List[str]:
        ...
        for i in range(len(self.evidence) - 1):
            ...
            condition += (
                polar_variable_mapping[ev_var] + " == " + str(domain_value) + " && "
            )
        ...
        code.append(
            f"\t{self.inference_name} = {polar_variable_mapping[self.target_variable]} * {self.indicator_name}"
        )
        return code

    def generate_query(self, network, polar_variable_mapping):
        return [
            f"E({self.inference_name}**{self.target_power})",
            f"E({self.indicator_name})",
        ]
    ...
    def generate_result(self, results):
        ...
        for index, domain_element in enumerate(
            self.network.variables[self.target_variable].domain
        ):
