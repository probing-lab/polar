...
class NetworkTransformer(Transformer):
    ...
    def start(self, args):
        ...
        variables: List[BayesVariable] = [
            v for v in args[1:] if isinstance(v, BayesVariable)
        ]
        for v in variables:
            if network.has_variable(v.name):
                raise BifFormatException(
                    f"Variable {v.name} is defined multiple times!"
                )
    ...
    def variable_block(self, args):
        ...
        if len(domain) == 0:
            raise BifFormatException(
                f"Variable {variable_name} has no type definitions."
            )
        elif len(domain) > 1:
            raise BifFormatException(
                f"Variable {variable_name} has multiple type definitions."
            )
    ...
    def type(self, args):
        ...
        if len(domain) != len(set(domain)):
            ...
            raise BifFormatException(
                "Defined type domain size does not agree with actual domain size."
            )
    ...
    def __add_cpt__(self, network: BayesNetwork, cpt):
        ...
        for parent in parent_names:
            if not network.has_variable(parent):
                raise BifFormatException(
                    f"Variable {parent} is parent of variable {name} but not defined!"
                )
        ...
        for type, element in cpt_entries:
            if type == AttributeType.DEFAULT:
                if default is not None:
                    raise BifFormatException(
                        f"CPT of Variable {variable.name} has multiple default-declarations."
                    )
                default = element
            elif type == AttributeType.TABLE:
                if table is not None:
                    raise BifFormatException(
                        f"CPT of Variable {variable.name} has multiple table-declarations."
                    )
                table = element
                ...
                if condition in condition_entries:
                    raise BifFormatException(
                        f"CPT of Variable {variable.name} has double entry for {condition}."
                    )
    ...
    def __add_default__(self, variable: BayesVariable, default: Tuple[float] = None):
        if default is None:
            ...
            raise BifFormatException(
                f"Default entry of variable {variable.name} does not cover all variable values."
            )
        elif not self.network.cpt_entry_sum_valid(default):
            raise BifFormatException(
                f"Default entry of variable {variable.name} does not sum up to 1."
            )
    ...
    def __add_table__(self, variable: BayesVariable, table: Tuple[float]):
        if table is None:
            return
        cpt_num_rows = reduce(
            lambda x, y: x * y, [p.domain_size for p in variable.parents], 1
        )
        if len(table) != variable.domain_size * cpt_num_rows:
            raise BifFormatException(
                f"Table entry for variable {variable.name} does not cover all CPT rows."
            )
        ...
        for row, condition in enumerate(product(*parent_domains)):
            ...
            if not self.network.cpt_entry_sum_valid(probs):
                raise BifFormatException(
                    f"Entry {condition} of variable {variable.name} does not sum up to 1."
                )
            variable.cpt_set_entry(condition, tuple(probs))

    def __add_entry__(
        self, variable: BayesVariable, condition: Tuple[str], probs: Tuple[float]
    ):
        name = variable.name
        if len(condition) != variable.num_parents:
            raise BifFormatException(
                f"Entry {condition} of variable {name} has invalid condition length."
            )
        elif len(probs) != variable.domain_size:
            raise BifFormatException(
                f"Entry {condition} of variable {name} has invalid number of values."
            )
        elif not self.network.cpt_entry_sum_valid(probs):
            raise BifFormatException(
                f"Entry {condition} of variable {name} does not sum up to 1."
            )
        else:
