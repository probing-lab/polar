...
class StatesPlot(Plot):
    ...
    def _build(self):
        ...
        if self.first_moment is not None:
            expectations = [
                float(eval_re(n, self.first_moment)) for n in range(number_iter)
            ]
        stds = None
        if self.is_probabilistic and self.second_moment is not None:
            second_moments = [
                float(eval_re(n, self.second_moment)) for n in range(number_iter)
            ]
            stds = [
                (second_moments[n] - (expectations[n] ** 2)) ** (1 / 2)
                for n in range(number_iter)
            ]
            stds = [
                (expectations[n] - 2 * stds[n], expectations[n] + 2 * stds[n])
                for n in range(number_iter)
            ]
        ...
        def prepare_animation(bar_container, expectation_line, std_line_0, std_line_1):
            def animate(frame_number):
                ...
                if expectation_line:
                    expectation_line.set_xdata(
                        [expectations[frame_number], expectations[frame_number]]
                    )
        ...
        _, _, bar_container = ax.hist(
            data[0], HIST_BINS, lw=1, ec="yellow", fc="blue", alpha=0.5
        )
        ...
        std_line_1 = None
        if stds:
            std_line_0 = ax.axvline(
                x=stds[0][0], linestyle="dotted", color="red", linewidth=2
            )
            std_line_1 = ax.axvline(
                x=stds[0][1], linestyle="dotted", color="red", linewidth=2
            )
            labels.append(
                Line2D(
                    [0],
        ...
        self.ani = animation.FuncAnimation(
            ...
            func=prepare_animation(
                bar_container, expectation_line, std_line_0, std_line_1
            ),
            interval=int(1000 / self.fps),
            frames=number_rendered_frames,
            repeat=False,
        )
