...
from symengine.lib.symengine_wrapper import Expr, Symbol, sqrt, Zero, One, factorial, sympy2symengine
import sympy as sym
...
class GramCharlierExpansion:
    ...
    def __call__(self):
        ...
        for i in range(3, count_cumulants+1):
            bell_args = [Zero(), Zero()] + [sympy2symengine(self.cumulants[j]) for j in range(3, i+1)]
        ...
        phi = sym.stats.density(sym.stats.Normal("_", sym.sympify(mu), sym.sympify(sigma)))(sym.sympify(x))
