...
from termination.polynomial.polynomial_termination_condition import (
    PolynomialTerminationCondition,
)
...
from termination.variance_based.variance_based_termination_analyzer import (
    VarianceBasedTerminationAnalyzer,
)
...
class TerminationAnalyzer:
    ...
    def analyze(
        ...
        if type(loop_guard) != Atom:
            print(
                "At the moment only loop guard consisting of a single condition are supported"
            )
            return

        poly, terminates_zero, terminates_negative = cls._normalize_atom(loop_guard)

        closed_form_poly = poly.expand().subs(
            cls._compute_closed_form_of_polynomial(
                poly.free_symbols, normalized_program
            )
        )
        ...
        if amber:
            ...
            branches = cls._compute_branches_for_polynomial(
                list(normalized_program.variables) + [poly], normalized_program
            )
            print(branches)

            bounds = cls._compute_bounds_form_polynomial(
                poly, branches, normalized_program
            )
            print(bounds)
        elif smt:
            has_prolog = (
                normalized_program.initial is not None
                and len(normalized_program.initial) > 0
            )
            formula = SMTTerminationCondition(
                closed_form_poly, terminates_zero, terminates_negative, has_prolog
            ).get_smt_formula()
            ...
            lc_recurrence = cls._compute_branches_for_polynomial(
                [poly], normalized_program
            )[poly]
            ...
            witness = analyzer.compute_bound(exact, genetic_algorithm_config, seed=seed)
            witness.print()
        else:
            witness = PolynomialTerminationCondition(
                closed_form_poly, terminates_zero, terminates_negative
            ).get_witness()
            if witness is None:
    ...
    @classmethod
    def _get_nondeterministic_branches(
        cls, branches: Dict[Monomial, List[Tuple[Expr, Expr]]], dist_vars: List[Symbol]
    ):
        ...
        while len(old_nondeterministic_vars) != len(nondeterministic_vars):
            ...
            for monom in branches:
                ...
                for prob_expr, eval_expr in branches[monom]:
                    if (
                        eval_expr.free_symbols
                        and len(eval_expr.free_symbols & nondeterministic_vars) > 0
                    ):
                        nondeterministic_vars.add(monom)
        return nondeterministic_vars

    @classmethod
    def _compute_bounds_form_polynomial(
        cls,
        poly: Poly,
        ...
        for assignment in program.loop_body:
            if isinstance(assignment, DistAssignment):
                assert (
                    assignment.variable not in dist_assignments
                ), "Program not in single-assignment form"
                dist_assignments[assignment.variable] = assignment

        rec_builder = RecBuilder(program)
        initial_values = rec_builder.get_initial_values(program.variables)
        nondeterministic_vars = cls._get_nondeterministic_branches(
            branches, dist_assignments.keys()
        )
        symbols = set().union(
            *[
                det_monom.free_symbols
                for det_monom in branches.keys() - nondeterministic_vars
            ]
        )

        closed_forms = cls._compute_closed_form_of_polynomial(symbols, program)

        deterministic_closed_forms = [
            det_monom.subs(closed_forms)
            for det_monom in branches.keys() - nondeterministic_vars
        ]
        bounds = cls._compute_bounds_of_expr(
            poly, branches, dist_assignments, deterministic_closed_forms, initial_values
        )
        return bounds
    ...
    def _compute_bounds_of_expr(
        ...
        branches = {
            sympify(b): [(sympify(ex1), sympify(ex2)) for ex1, ex2 in branches[b]]
            for b in branches
        }
        dist_assignments = {sympify(s): dist_assignments[s] for s in dist_assignments}
        bounds = compute_bounds_of_expr(
            poly, branches, dist_assignments, closed_forms, initial_values
        )
        return bounds
    ...
    @classmethod
    def _compute_closed_form_of_polynomial(
        cls, symbols: List[Symbol], program: Program
    ):
        ...
        for symbol in symbols:
            ...
            closed_forms[symbol1], is_exact = recurrence_builder.get_solution(
                symbol, solvers
            )
            if not is_exact:
