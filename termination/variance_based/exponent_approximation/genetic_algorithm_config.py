...
class MinMaxGeneticAlgorithmConfig(GeneticAlgorithmConfig):
    def __init__(
        ...
        self.granularity_coeff = (
            max_granularity - min_granularity
        ) / num_iterations**degree_gran

        self.max_population = max_population
        self.population_coeff = (
            min_population - max_population
        ) / num_iterations**degree_pop
    ...
    def get_granularity(self, iteration):
        gr = int(
            ...
            + (iteration + 1) ** self.degree_gran * self.granularity_coeff
        )
        return gr
    ...
    def get_population_size(self, iteration):
        return int(
            self.max_population
            + self.population_coeff * (iteration + 1) ** self.degree_pop
        )
