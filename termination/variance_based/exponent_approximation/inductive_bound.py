...
def init(solver_name: str):
    ...
    solver = pywraplp.Solver.CreateSolver(
        solver_name
    )  # CLP seems to have better numeric stability (e.g. not so often "ABNORMAL" result) than GLOP
    if solver_name == "GUROBI":
...
def _get_solution(
    ...
    exponent = (
        -(
            (
                sub_gaussian_cutoff
                - b
                - delta_1 * (sqrt(1 + var_scaling) / (sqrt(1 + var_scaling) - 1))
            )
            ** 2
        )
        / 2
    ...
    constraint = solver.Constraint(
        -infinity, (-epsilon * SCALING), f"p_q_translation_leftmost"
    )
...
def _check_model(
    ...
    return _get_solution(
        list(np.linspace(0, specification_end, granularity)),
        epsilon,
        d,
        specification_end + sg_cutoff,
        C,
        delta_1,
        c_0,
        b,
...
class ParameterSelector:
    ...
    def get_epsilon(self):
        if 0 in self.play_count:
            ...
            idx = np.argmax(
                self.exponent_sum / np.maximum(self.play_count, 1) / (self.max_reward)
                + np.sqrt(2 * np.log(self.n) / np.maximum(self.play_count, 1))
            )
        range = self.EMAX - self.EZERO
        return self.random_generator.uniform(
            idx / self.NUM_BINS * range, (idx + 1) / self.NUM_BINS * range
        )


def estimate_bound_exponent_inductive_bound(
    degree: float,
    C: float,
    delta_1: float,
    delta_2: float,
    ...
    for i in range(num_random_choices):
        ...
        res_min = _check_if_model_exists(
            d_current, epsilon, C, delta_2, c_0, granularity
        )
        if res_min == None:
