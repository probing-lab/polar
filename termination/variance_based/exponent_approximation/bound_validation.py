...
def validate_bound(values: np.ndarray, quantiles: np.ndarray, epsilon: float, d: float, sg_cutoff: float, C: float, c_0: float, delta1: float):
    assert len(values) == len(quantiles)
    assert d > 0
    assert epsilon > 0 and epsilon < 1

    b = _compute_b(epsilon, d, C)
    sg_value = exp(-C*(sg_cutoff-delta1*(sqrt(1+d)/(sqrt(1+d))) - b)**2/2)
