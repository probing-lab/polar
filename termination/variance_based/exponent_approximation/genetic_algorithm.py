...
class GeneticAlgorithm:
    def __init__(self, C:float, delta_1: float, delta_prime:float, c_0:float, degree:float, seed=None):
        self.C = C
    ...
    def fitness(self, spec: InductiveBoundSpecification):
        try:
            if _check_model(spec.d, spec.epsilon, self.C, self.delta_1, self.c_0, 
...
def estimate_bound_exponent_inductive_bound_genetic(degree: float, C: float, delta_prime: float, delta_1: float, c_0: float, n_0, granularity: int = 20, pop_size: int=30, pop_multiplier: int=5, num_generations=5, seed=0):
    ...
    Args:
        ...
        delta_prime (float): multiplicative error for k^{degree}
        delta_1 (float): error for the tail-bound offset
