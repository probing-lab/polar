...
from termination.variance_based.exponent_approximation.bound_validation import (
    validate_bound,
)
from termination.variance_based.exponent_approximation.closed_form_bound import (
    _get_c_d_prime,
    get_closed_form_bound_asymptotic,
)
from termination.variance_based.exponent_approximation.converging_constants import (
    compute_c_0,
    compute_delta_cb,
    compute_delta_prime,
    get_k_delta,
...
from termination.variance_based.exponent_approximation.inductive_bound import (
    PrecisionException,
    _check_model,
    _compute_b,
    init,
...
class InductiveBoundSpecification:
    ...
    def __eq__(self, value):
        return (
            ...
            and value.d == self.d
            and value.epsilon == self.epsilon
            and value.granularity == self.granularity
            and value.specification_end == self.specification_end
            and value.sg_cutoff == self.sg_cutoff
        )
...
class GeneticAlgorithm:
    ...
    def _get_delta_1(self, n0):
        if self.q1 is None or self.q2 is None:
            return 1e-8
        return compute_delta_cb(
            n0, self.p, self.q1.as_expr(), self.q2.as_expr(), self.initial_value
        )
    ...
    def _get_c_0(self, n0, k):
        if self.q1 is None or self.q2 is None:
            return 1e-8
        return compute_c_0(
            n0, self.p, self.q1.as_expr(), self.q2.as_expr(), k, self.initial_value
        )

    def _get_n0_from_c0(self, c0):
        if self.q1 is None or self.q2 is None:
            return 0
        return get_n0_from_c0(
            self.p, self.q1.as_expr(), self.q2.as_expr(), c0, self.initial_value
        )
    ...
    def fitness(self, spec: InductiveBoundSpecification):
        try:
            ...
            if (
                ...
                exponent_fitness = log(1 - spec.epsilon) / log(
                    k + k_delta
                ...
                try:
                    if (
                        ...
                        series_sum = (
                            zeta(-exponent_fitness, spec.n0 + 1) * coeff + spec.n0
                        )  # TODO: The zeta function is an over-approximation, because actually the elements are bounded by 1 from above (would need the partial sum from n_0).
                        abs_bound = series_sum
                    return (
                        abs_bound,
                        exponent_fitness,
                        coeff,
                    )  # fitness has two dimensions: the first is the actual bound (absolute value), the second is the exponent
            ...
            ValueError
        ) as er:  # This error can occure in the computation of constants
            return (np.infty, 0, np.infty)
    ...
    def get_best_bound(self):
        ...
        return (
            ...
            spec.epsilon,
            spec.d,
            spec.specification_end + spec.sg_cutoff,
            c_0,
            delta_1,
        )
    ...
    def mutate(self, spec: InductiveBoundSpecification, new_granularity):
        ...
        if self.rand_gen.random() < 0.6:
            n0 *= (
                ...
                + 0.1
                + (0.5 if self.fitness(spec)[0] == np.infty else 0)
            )

        return InductiveBoundSpecification(
            n0, d, epsilon, granularity, specification_end, sg_cutoff
        )

    def crossover(
        self,
        spec1: InductiveBoundSpecification,
        spec2: InductiveBoundSpecification,
        new_granularity,
    ):
        ...
        specification_end = (
            spec1.specification_end
            if self.rand_gen.random() < 0.5
            else spec2.specification_end
        )
        sg_cutoff = spec1.sg_cutoff if self.rand_gen.random() < 0.5 else spec2.sg_cutoff

        return InductiveBoundSpecification(
            n0, d, epsilon, new_granularity, specification_end, sg_cutoff
        )
    ...
    def get_initial_guesses(self, granularity, size, adapt_d=True):
        ...
        self.population = []
        c_prime, d_prime = _get_c_d_prime(
            self.C
        )  # this serves just as a heuristic, to always guess in the somewhat right area
        for _ in range(size):
    ...
    def get_new_population(
        self, mutation_multipier, crossover_multiplier, new_granularity
    ):
        elems = self.rand_gen.choice(
            self.population, mutation_multipier * len(self.population), replace=True
        )
        mutations = [self.mutate(spec, new_granularity) for spec in elems]

        crossover_parents = [
            tuple(self.rand_gen.choice(self.population, size=2, replace=False))
            for _ in range(int(crossover_multiplier * len(self.population)))
        ]
        children = [
            self.crossover(p1, p2, new_granularity) for p1, p2 in crossover_parents
        ]
        self.population = self.population + mutations + children

    def sort_population(self):
        self.population = sorted(
            self.population, key=lambda spec: self.fitness(spec), reverse=False
        )
    ...
    def print_best(self):
        print(
            f"exponent: {self.fitness(self.population[0])},epsilon: {self.population[0].epsilon}, d: {self.population[0].d}, spec_end: {self.population[0].specification_end}, sg_cutoff: {self.population[0].sg_cutoff}, n0: {self.population[0].n0}"
        )


def _get_poly_degree(q):
    def extract_exponent(term, symbol):
        _, exponent = term.as_coeff_exponent(symbol)
        return exponent

    if q.is_Add:
        return max(extract_exponent(arg, N) for arg in q.args)
    else:
        return extract_exponent(q, N)


def estimate_bound_exponent_inductive_bound_genetic(
    p: float,
    ...
    if (initial_expr is None or not sympify(initial_expr).is_number) and exact_n0:
        raise Exception(
            "Can not compute exact bound for stopping time, when initial value of loop guard is unknown"
        )
    C = 4 * p * (1 - p)
    ...
    genetic_algorithm = GeneticAlgorithm(
        C,
        p,
        q_1 if exact_n0 else None,
        q_2 if exact_n0 else None,
        initial_expr,
        var_degree,
        seed,
    )
    genetic_algorithm.get_initial_guesses(
        algorithm_config.get_granularity(0), algorithm_config.get_population_size(0)
    )
    genetic_algorithm.sort_population()
    for i in range(algorithm_config.get_num_iterations()):
        print(
            f"Starting generation {i} with best element. Gen_size: {len(genetic_algorithm.population)}, granularity:{algorithm_config.get_granularity(i)}:"
        )
        genetic_algorithm.print_best()
        genetic_algorithm.get_new_population(
            algorithm_config.get_population_mutation_multiplier(i),
    ...
    bound_quantiles, bound_vals, epsilon, d, sg_cutoff, c_0_res, delta_1_res = (
        genetic_algorithm.get_best_bound()
    )
    inductive_valid, initial_valid = validate_bound(
        bound_vals, bound_quantiles, epsilon, d, sg_cutoff, C, c_0_res, delta_1_res
    )
    assert (
        inductive_valid
    ), "inductive validity of bound could not be verified (probably a numeric error)"
    assert initial_valid, "initial validity of bound could not be verified"

    return VarianceBoundWitness(
        genetic_algorithm.fitness(genetic_algorithm.population[0])[1],
        (
            Symbol("B")
            if not exact_n0
            else genetic_algorithm.fitness(genetic_algorithm.population[0])[2]
        ),
