...
import numpy as np
from termination.variance_based.exponent_approximation.genetic_algorithm import (
    estimate_bound_exponent_inductive_bound_genetic,
)
...
from termination.variance_based.exponent_approximation.inductive_bound import (
    estimate_bound_exponent_inductive_bound,
)
...
class VarianceBasedTerminationAnalyzer:
    def __init__(self, p1, q1, p2, q2, initial_value):
        self.p1 = p1
        self.q1 = q1.subs({Symbol("n", integer=True): N}).as_expr()
        self.p2 = p2
        self.q2 = q2.subs({Symbol("n", integer=True): N}).as_expr()
        self.initial_value = initial_value
        assert self.q1.free_symbols == set() or self.q1.free_symbols == set(
            [N]
        ), "Only 'n' may occur in polynomial q1"
        assert self.q1.free_symbols == set() or self.q2.free_symbols == set(
            [N]
        ), "Only 'n' may occur in polynomial q2"
    ...
    def compute_bound(
        self, exact, genetic_algorithm_config: GeneticAlgorithmConfig, seed=None
    ):
        ...
        assert (
            max_degree_q1 == max_degree_q2
            and max_coeff_p1 * self.p1 + max_coeff_p2 * self.p2 == S.Zero
        ), "Degree of expected value of loop guard change not lower than twice the degree of the variance."

        witness = estimate_bound_exponent_inductive_bound_genetic(
            self.p1,
            genetic_algorithm_config,
            self.q1,
            self.q2,
            initial_expr=self.initial_value,
            exact_n0=exact,
            seed=seed,
        )
