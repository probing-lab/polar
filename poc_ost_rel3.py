...
program = Parser().parse_file("documentation/loops_ost_extension/related_work_3.prob")
...
compute_bounds(random_vars,
               ...
               4,
               recurrence_builder,
               [(Symbol("l0", real=True, finite=True, positive=True),sympify(0), oo)],
               {Symbol("l", real=True): sympify(-9), Symbol("k", real=True):sympify(1)},
               {Symbol("l", real=True): sympify(0)})
